...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = {
    ...
    if(right.isEmpty && left.isEmpty) then elem
    else if (right.isEmpty && !left.isEmpty) then max(elem, left.mostRetweeted)
    else if (!right.isEmpty && left.isEmpty) then max(elem, right.mostRetweeted)
    else {
  ...
  def descendingByRetweet: TweetList = {
    val me = mostRetweeted
    new Cons(me, remove(me).descendingByRetweet)
  }
  /**
   * The following methods are already implemented
   */
